...
PROG=samba      # App name
VER=4.5.0      # App version
VERHUMAN=$VER   # Human-readable version
#PVER=          # Branch (set in config.sh, override here if needed)
PKG=$PKGNAMESPACE/service/network/samba45 # Package name (e.g. library/foo)
SUMMARY="${PROG}45 - CIFS server and domain controller"
...
service_configs() {
    ...
    logcmd mkdir -p $DESTDIR/lib/svc/manifest/network/samba45
    # can't use logmsg here (screws with stdin/stdout/stderr)
    sed "s@{{PREFIX}}@${PREFIX}@g" \
        $SRCDIR/files/smf/manifest-samba-smbd.xml.tmpl \
        > $DESTDIR/lib/svc/manifest/network/samba45/smbd.xml
    sed "s@{{PREFIX}}@${PREFIX}@g" \
        $SRCDIR/files/smf/manifest-samba-nmbd.xml.tmpl \
        > $DESTDIR/lib/svc/manifest/network/samba45/nmbd.xml
    sed "s@{{PREFIX}}@${PREFIX}@g" \
        $SRCDIR/files/smf/manifest-samba-winbindd.xml.tmpl \
        > $DESTDIR/lib/svc/manifest/network/samba45/winbindd.xml
}
