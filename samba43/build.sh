...
service_configs() {
    ...
    logcmd mkdir -p $DESTDIR/lib/svc/manifest/network/samba43
    # can't use logmsg here (screws with stdin/stdout/stderr)
    sed "s@{{PREFIX}}@${PREFIX}@g" \
        $SRCDIR/files/smf/manifest-samba-smbd.xml.tmpl \
        > $DESTDIR/lib/svc/manifest/network/samba43/smbd.xml
    sed "s@{{PREFIX}}@${PREFIX}@g" \
        $SRCDIR/files/smf/manifest-samba-nmbd.xml.tmpl \
        > $DESTDIR/lib/svc/manifest/network/samba43/nmbd.xml
    sed "s@{{PREFIX}}@${PREFIX}@g" \
        $SRCDIR/files/smf/manifest-samba-winbindd.xml.tmpl \
        > $DESTDIR/lib/svc/manifest/network/samba43/winbindd.xml
}
