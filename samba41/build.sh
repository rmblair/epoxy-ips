...
PROG=samba      # App name
VER=4.1.22      # App version
VERHUMAN=$VER   # Human-readable version
#PVER=          # Branch (set in config.sh, override here if needed)
PKG=$PKGNAMESPACE/service/network/samba41 # Package name (e.g. library/foo)
SUMMARY="${PROG}41 - CIFS server and domain controller"
...
BUILD_DEPENDS_IPS="system/library/gcc-4-runtime system/library/mozilla-nss \
    library/nspr system/library/security/libsasl library/zlib runtime/perl"
...
service_configs() {
    ...
    logcmd mkdir -p $DESTDIR/lib/svc/manifest/network/samba41
    # can't use logmsg here (screws with stdin/stdout/stderr)
    sed "s@{{PREFIX}}@${PREFIX}@g" \
        $SRCDIR/files/smf/manifest-samba-smbd.xml.tmpl \
        > $DESTDIR/lib/svc/manifest/network/samba41/smbd.xml
    sed "s@{{PREFIX}}@${PREFIX}@g" \
        $SRCDIR/files/smf/manifest-samba-nmbd.xml.tmpl \
        > $DESTDIR/lib/svc/manifest/network/samba41/nmbd.xml
    sed "s@{{PREFIX}}@${PREFIX}@g" \
        $SRCDIR/files/smf/manifest-samba-winbindd.xml.tmpl \
        > $DESTDIR/lib/svc/manifest/network/samba41/winbindd.xml
}
